...
Robots use **Breadth-First Search** instead of greedy movement, which automatically handles wall avoidance and zone boundaries:
...
The `relaxed=True` parameter authorizes the robot to step one column past its zone limit, exclusively when dropping the transformed waste at the boundary.
